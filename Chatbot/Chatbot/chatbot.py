...
from KimmmmmchiDjango.settings import OPENAI_API_KEY, PORTKEY_API_KEY
import logging
...
class Chatbot:
  def __init__(self):
    ...
    loader = Loader()
    docs = loader.invoke('Chatbot/Chatbot/Data')
    logging.info(f"{len(docs)} documents loaded successfully.")

    self.retriever = createRetriever(self.retrievalModel ,docs)
  ...
  async def ainvoke(self, query, session_id):
    try:
      logging.info(f"Received query: {query} for session {session_id}")
